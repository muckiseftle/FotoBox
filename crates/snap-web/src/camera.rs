...
//!
//! App-level capture options (countdown length, beep, selfie mirror) that work
//! with both the mock and real backends. Hardware parameters (ISO, aperture,
//! shutter via gphoto2) are added here on the Ubuntu target.
//!
//! `GET /api/camera` is public (the kiosk needs the countdown/mirror settings);
//! changes and re-detect require an admin.
...
/// Public: live camera status plus the capture settings the kiosk applies.
